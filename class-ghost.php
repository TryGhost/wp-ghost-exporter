...
class Ghost {
	...
	private function __construct() {
		...
		if ( isset( $_GET['ghostarchiveexport'] ) ) {
			add_action( 'init', array( $this, 'download_archive' ) );
		}
	...
	public function download_file() {
		...
		$filedir = $upload_dir['basedir'] . '/ghost-exports';
		if ( ! file_exists( $filedir ) ) {
        wp_mkdir_p( $filedir );
    }
		$filename = 'wp_ghost_export_' . time() . '.json';
	...
	/**
	 * Generate a zip archive of images in a Ghost compatible directory structure.
	 * @return bin file
	 */
	public function download_archive() {
	  ...
	  // Set the path to upload directory
  	$upload_dir = wp_upload_dir();
	  ...
	  // Initialise the archive object
		$gziparchivename = 'wp_ghost_image_archive.zip';
